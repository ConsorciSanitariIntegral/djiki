...
def view(request, title, revision_pk=None):
    ...
    try:
        ...
        html = _templating.render_to_string(
            'djiki/not_found.html', {'title': page_title}, request)
    ...
    if request.REQUEST.get('raw', ''):
        ...
        response[
            'Content-Disposition'] = 'attachment; filename=%s.txt' % quote(title.encode('utf-8'))
...
def revert(request, title, revision_pk):
    ...
    if not auth.can_edit(request, page):
        raise PermissionDenied
    src_revision = get_object_or_404(
        models.PageRevision, page=page, pk=revision_pk)
