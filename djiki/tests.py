...
class SimpleTest(TestCase):
    ...
    def _page_edit(self, title, content, description='', username=None, password=None):
        ...
        rev_count = models.PageRevision.objects.filter(
            page__title=title).count()
        try:
            prev_rev = models.PageRevision.objects.filter(
                page__title=title).order_by('-created')[0].pk
        except IndexError:
            prev_rev = ''
    ...
    def test_underscores(self):
        ...
        r = client.get(
            reverse('djiki-page-view', kwargs={'title': title_xlat}))
    ...
    def test_history_view(self):
        ...
        r = self.anon_client.get(
            reverse('djiki-page-history', kwargs={'title': title}))
        self.assertEqual(r.status_code, 200)
