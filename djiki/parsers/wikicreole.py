...
class DjikiHtmlEmitter(HtmlEmitter):
    image_params_re = re.compile(
        r'^(?:(?P<size>[0-9]+x[0-9]+)(?:\||$))?(?P<title>.*)$')
    ...
    def link_emit(self, node):
        ...
        return u'<a href="%s">%s</a>' % (
            reverse(
                'djiki-page-view', kwargs={'title': utils.urlize_title(self.attr_escape(target))}),
            inside)
        ...
        if m:
            ...
            try:
                image = models.Image.objects.get(
                    name=utils.deurlize_title(target))
                ctx['image'] = image
